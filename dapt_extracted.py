...
# Checkpoint policy:
# - canonical checkpoint is OVERWRITTEN every 10 completed epochs
# - an emergency checkpoint is written at the 11-hour wall-clock guard
# - a new session can attach either checkpoint as an input; Cell 3 auto-discovers
#   the highest completed epoch and resumes from there.
CHECKPOINT_INTERVAL = 10
CKPT = f"/kaggle/working/{RUN_TAG}_resume.pth"
EMERGENCY_CKPT = f"/kaggle/working/{RUN_TAG}_emergency.pth"
LEGACY_CKPT = f"/kaggle/working/{RUN_TAG}_last.pth"
CHECKPOINT_MANIFEST = f"/kaggle/working/{RUN_TAG}_checkpoint_status.json"
FINAL_ENCODER = f"/kaggle/working/{RUN_TAG}_encoders.pth"
...
# The architecture's original P2 plan quoted a rough ~3.5 h budget for the
# original 6,800-pair DAPT run. Use this ONLY as an initial baseline; Cell 4
# replaces it with measured epoch timings for the actual 6,664-pair run.
...
ax.axis("off")
boxes = [
    (0.03, 0.57, "Epochs 1–10", "Train\nthen save"),
    (0.25, 0.57, "Checkpoint", "geonexus_v3_2_dapt_6664_resume.pth\noverwrite previous"),
    (0.52, 0.57, "Epochs 11–20", "Train\nthen save"),
    (0.74, 0.57, "...", "repeat every 10"),
    (0.03, 0.12, "Session ends", "Kaggle timeout / stop"),
    (0.31, 0.12, "Attach checkpoint", "new Kaggle session\nInput"),
    (0.60, 0.12, "Auto-discover", "choose highest\ncompleted epoch"),
    (0.82, 0.12, "Resume", "next epoch\nfrom saved state"),
]
for x, y, title, body in boxes:
    ax.text(x, y, title, ha="center", va="center", fontsize=13, fontweight="bold",
            bbox=dict(boxstyle="round,pad=0.65", facecolor="white", edgecolor="black", linewidth=1.5))
    ax.text(x, y-0.09, body, ha="center", va="center", fontsize=10, family="monospace")
for a,b in [((0.11,0.57),(0.20,0.57)), ((0.33,0.57),(0.46,0.57)), ((0.60,0.57),(0.69,0.57)),
            ((0.11,0.12),(0.25,0.12)), ((0.38,0.12),(0.54,0.12)), ((0.67,0.12),(0.77,0.12))]:
    ax.annotate("", xy=b, xytext=a, arrowprops=dict(arrowstyle="->", linewidth=2))
ax.text(0.5, 0.93, "Geo-Nexus v3.2 — 6664-pair DAPT checkpoint/resume protocol",
        ha="center", fontsize=16, fontweight="bold")
ax.text(0.50, 0.02,
        "Canonical checkpoint: every 10 epochs | Emergency checkpoint: at wall-clock guard | Saved under /kaggle/working",
        ha="center", fontsize=10)
plt.tight_layout()
plt.savefig(CHECKPOINT_FLOW_FIGURE, dpi=220, bbox_inches="tight")
plt.show()

print(f"Checkpoint workflow image saved to:\n{CHECKPOINT_FLOW_FIGURE}")
print("CELL 1 COMPLETE — paths and checkpoint policy are configured.")
...
def checkpoint_candidates():
    patterns = [
        ...
        f"{RUN_TAG}_last.pth",  # legacy notebook checkpoint
    ]
    roots = [Path("/kaggle/working"), INPUT_ROOT]
    found = []
    for root in roots:
        if not root.exists():
            continue
...
#            checkpoint every 10 epochs + emergency checkpoint
#            + automatic resume + live timing / ETA
# ============================================================
...
# Epoch durations from this session only. Once we have measurements, ETA is
# computed from the observed median of recent completed epochs.
epoch_durations = []


def out_of_time():
    return (time.time() - START_TIME) > WALL
...
def checkpoint_state(completed_epochs, reason, started_at, finished_at):
    return {
        ...
        "epoch": int(completed_epochs - 1),  # legacy-compatible field
        "hist": hist,
        "cfg": CFG,
        "rng": save_rng_state(),
        "save_reason": reason,
        "session_started_ist": started_at,
        "checkpoint_saved_ist": finished_at,
    }


def save_checkpoint(path, completed_epochs, reason):
    ...
    status = {
        ...
        "reason": reason,
        "path": str(path),
        "saved_at_ist": started_at,
    }
    with open(CHECKPOINT_MANIFEST, "w", encoding="utf-8") as f:
        json.dump(status, f, indent=2)

    print(f"CHECKPOINT SAVED: epoch {completed_epochs}/{CFG['epochs']} | {reason}")
    print(f"  file: {path}")


# If resumed from an earlier checkpoint, epoch_duration history may be stored
# in the checkpoint history as optional fields from previous sessions.
for row in hist:
    if "epoch_seconds" in row and row["epoch_seconds"] > 0:
        epoch_durations.append(float(row["epoch_seconds"]))

completed = START_COMPLETED_EPOCHS >= CFG["epochs"]

if completed:
    print("DAPT is already complete according to the loaded checkpoint.")
    if not os.path.exists(FINAL_ENCODER):
        ...
        print(f"Final encoder restored from checkpoint: {FINAL_ENCODER}")
else:
    print("=" * 78)
    print("DAPT TRAINING START")
    print("=" * 78)
    print("Current time (IST):", now_ist().strftime("%Y-%m-%d %H:%M:%S %Z"))
    print("Device             :", DEV)
    print("GPUs detected      :", torch.cuda.device_count())
    print("Training uses      : GPU 0 (single-device implementation)")
    print("Starting after     :", START_COMPLETED_EPOCHS, "completed epochs")
    print("Checkpoint policy  : every 10 epochs, overwrite canonical checkpoint")
    print("Wall-clock guard   : 11 hours")

for ep_idx in range(START_COMPLETED_EPOCHS, CFG["epochs"]):
    if out_of_time():
        print("Wall-clock guard reached before starting the next epoch.")
        break

    epoch_num = ep_idx + 1
    epoch_start = time.time()

    # The derived stem is trained without weight decay for epochs 1–10, then
    # weight decay is restored starting with epoch 11.
    if epoch_num == 11 and opt.param_groups[1]["weight_decay"] == 0.0:
        opt.param_groups[1]["weight_decay"] = CFG["weight_decay"]
        print("Derived-stem weight decay re-enabled at epoch 11.")
    ...
    # Exactly one scheduler step after each completed epoch.
    sched.step()
    ...
    record = {
        ...
        **metrics,
        ...
        "lr_derived": opt.param_groups[1]["lr"],
        "lr_head": opt.param_groups[2]["lr"],
        "time_ist": now_ist().isoformat(),
    }
    hist.append(record)

    # Keep the small history JSON current every epoch. The heavy model state is
    # checkpointed only at the requested 10-epoch boundaries (plus emergency).
    with open(HISTORY_FILE, "w", encoding="utf-8") as f:
        json.dump(hist, f, indent=2)
    ...
    median_epoch = float(np.median(recent)) if recent else float(epoch_seconds)
    avg_epoch = float(np.mean(recent)) if recent else float(epoch_seconds)
    eta_seconds = median_epoch * remaining_epochs
    eta_time = now_ist() + timedelta(seconds=eta_seconds)

    print(
        f"epoch {epoch_num:03d}/{CFG['epochs']:03d} | "
        ...
        f"lr_backbone {record['lr_backbone']:.3e} | "
        f"lr_head {record['lr_head']:.3e}"
    )
    print(
        f"  epoch time={format_seconds(epoch_seconds)} | "
        f"recent median={format_seconds(median_epoch)} | "
        f"recent mean={format_seconds(avg_epoch)} | "
        f"remaining={remaining_epochs} | "
        f"ETA≈{eta_time.strftime('%H:%M:%S IST')} ({format_seconds(eta_seconds)})"
    )

    # Canonical resume checkpoint: overwrite the previous one every 10 epochs.
    if (completed_epochs % CHECKPOINT_INTERVAL == 0) or (completed_epochs == CFG["epochs"]):
        save_checkpoint(CKPT, completed_epochs, reason="10-epoch boundary" if completed_epochs % CHECKPOINT_INTERVAL == 0 else "final")

    if completed_epochs == CFG["epochs"]:
        completed = True
        break

    # Extra safety: if the runtime is close to the wall-clock limit, save an
    # emergency checkpoint at the latest completed epoch. This is separate from
    # the canonical 10-epoch checkpoint and is automatically discoverable next run.
    if out_of_time():
        save_checkpoint(EMERGENCY_CKPT, completed_epochs, reason="11-hour emergency")
        print("Wall-clock guard reached after a completed epoch.")
        print("Resume next time from the highest discovered checkpoint.")
        break


if completed:
    # Final encoder-only artifact: projectors are discarded after DAPT.
    torch.save(
        {
            "encoder": model.encoder.state_dict(),
            "cfg": CFG,
    ...
    print("=" * 78)
    print(f"Final encoder   : {FINAL_ENCODER}")
    print(f"Training history: {HISTORY_FILE}")
else:
    print("\nDAPT is not yet complete.")
    if os.path.exists(EMERGENCY_CKPT):
        print(f"Emergency checkpoint: {EMERGENCY_CKPT}")
    if os.path.exists(CKPT):
        print(f"Canonical checkpoint: {CKPT}")
    print("Attach the highest-epoch checkpoint as an input in the next session; Cell 3 will auto-discover it.")


# ============================================================
# CELL 5 — training curves + shareable diagnostics
# ============================================================
...
if hist:
    ...
    raise FileNotFoundError("No DAPT history is available yet.")
...
com = [h["com"] for h in history]
uni = [h["uni"] for h in history]
opt_loss = [h["opt"] for h in history]
sar_loss = [h["sar"] for h in history]
temp = [h["temporal"] for h in history]

fig, ax = plt.subplots(figsize=(12, 7))
ax.plot(epochs, total, label="total")
ax.plot(epochs, com, label="com")
ax.plot(epochs, uni, label="uni")
ax.plot(epochs, opt_loss, label="opt")
ax.plot(epochs, sar_loss, label="sar")
ax.plot(epochs, temp, label="temporal")
ax.set_title("Geo-Nexus v3.2 DAPT — Training Diagnostics")
ax.set_xlabel("Epoch")
ax.set_ylabel("Loss")
ax.grid(alpha=0.25)
ax.legend()
plt.tight_layout()
plt.savefig(LOSS_FIGURE, dpi=220, bbox_inches="tight")
plt.show()

print(f"Training curve image saved to: {LOSS_FIGURE}")
print(f"Recorded epochs: {len(history)}")
print(f"Last recorded epoch: {history[-1]['epoch'] + 1}")
if len(history) < CFG["epochs"]:
    print("STATUS: DAPT incomplete — resume from the highest checkpoint.")
else:
    print("STATUS: 100 epochs recorded.")
...
if not os.path.exists(FINAL_ENCODER):
    raise FileNotFoundError(
        "Final encoder was not found. Complete all 100 epochs first."
    )

# Reload encoder weights into the current model before diagnostics.
final_state = torch.load(FINAL_ENCODER, map_location="cpu")
model.encoder.load_state_dict(final_state["encoder"])
model.eval()
...
with torch.no_grad():
    for batch in tsne_loader:
        x = batch["x"][:, 0].to(DEV, non_blocking=True).float()

        zo, zs = model.project(x)

        remaining = MAX_EMBEDDINGS_PER_MODALITY - collected
        if remaining <= 0:
            break
        ...
        z_optical.append(zo.cpu().numpy())
        z_sar.append(zs.cpu().numpy())

        collected += zo.shape[0]

        if collected >= MAX_EMBEDDINGS_PER_MODALITY:
            break

zo = np.concatenate(z_optical, axis=0)
zs = np.concatenate(z_sar, axis=0)

if len(zo) < 2 or len(zs) < 2:
    raise RuntimeError("Not enough embeddings for t-SNE.")

combined = np.concatenate([zo, zs], axis=0)
...
ax.set_title("Geo-Nexus v3.2 DAPT — Optical vs SAR Embeddings")
...
print("=" * 78)
print(f"Embeddings per modality: {n}")
print(f"Saved figure           : {TSNE_FILE}")



# ============================================================
# CELL 7 — final integrity manifest
# ============================================================
...
if missing:
    ...
    print("All required diagnostic/history outputs are present.")

for p in required_outputs + optional_outputs:
    if os.path.exists(p):
        size_mb = os.path.getsize(p) / (1024**2)
        print(f"{Path(p).name:48s} | {size_mb:10.2f} MiB")
    elif p in optional_outputs:
        print(f"{Path(p).name:48s} | not created yet")

if os.path.exists(FINAL_ENCODER):
    encoder_sha = sha256_file(FINAL_ENCODER)
    print("\nFinal encoder SHA256:")
    print(encoder_sha)
    ...
    manifest = {
        ...
        "satara_pairs": 3349,
        "total_pairs": 6664,
        "stem_surgery_cosine": float(stem_cosine),
        "epochs_recorded": len(hist),
        "final_encoder": FINAL_ENCODER,
        "final_encoder_sha256": encoder_sha,
    }

    manifest_path = f"/kaggle/working/{RUN_TAG}_manifest.json"
...
if os.path.exists(FINAL_ENCODER):
    print("\nDAPT final artifact is ready for P3 OSCD fine-tuning.")
else:
    print("\nResume checkpointing is active; final encoder will be created after epoch 100.")
